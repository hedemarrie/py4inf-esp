...
Note, unlike in Python, in an SQL {\tt WHERE} clause 
...
the storage and processing time hardly matters.  But if we have 
...
name) and we need the {\tt id} value for the person, depending on whether or not the person is already
...
keep us from making mistakes and introducing incorrect data into 
our tables.   When we create our tables:
...
field \verb"from_id" in {\tt Follows} is the same {\tt id} value
...
browser ``locks'' the database file keeping any other program
from accessing the file.  In particular, your Python program
will not be able to access the file if it is locked.

So, a solution is to make sure to either close the database browser, or use the {\bf File} menu to close the database in the browser
